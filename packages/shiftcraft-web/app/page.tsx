...
import { DEFAULT_INPUT, DEFAULT_SETTINGS, loadSettings, newEmployee, saveSettings } from "@/lib/defaultInput";
import { exportScheduleCsv } from "@/lib/exportCsv";
import type { ScheduleInput, ScheduleResult, ScheduleSettings } from "@/lib/types";
...
export default function Home() {
  ...
  function handleSettingsImport(s: ScheduleSettings) {
    setSettings(s);
    saveSettings(s);
    setResult(null);
  }
  ...
  async function handleGenerate() {
    ...
    setLoading(true);
    try {
      setResult(await generateSchedule(settings, input));
    } catch (e) {
  ...
  function exportSettingsJson() {
    ...
    a.href = url;
    a.download = "shiftcraft-settings.json";
    a.click();
  ...
  const isSuccess = result && (result.status === "optimal" || result.status === "feasible");
  const firstDate = isSuccess ? Object.keys(result.schedule).sort()[0] : null;
  const hasSettings = settings.rules.length > 0 || settings.shifts.length > 0;
    ...
    <div className="flex flex-col h-screen overflow-hidden bg-background">
      ...
      <header className="border-b px-6 py-3 flex items-center justify-between shrink-0 bg-background z-10">
        ...
        <div className="flex items-center gap-2">
          {/* Settings controls */}
          <SettingsImport onImport={handleSettingsImport} />
          <Button variant="outline" size="sm" onClick={exportSettingsJson} className="gap-1.5">
            <FileJson className="h-3.5 w-3.5" /> Export Settings
          </Button>
          ...
          <Button variant="outline" size="sm" onClick={exportInputJson} className="gap-1.5">
            <FileJson className="h-3.5 w-3.5" /> Export Input
          </Button>
          ...
          <Button
            ...
            disabled={loading || input.team.length === 0}
            className="gap-1.5"
          >
            {loading ? (
              <>
      ...
      {/* Settings banner — shown when using defaults */}
      {!hasSettings && (
        <div className="border-b bg-amber-50 dark:bg-amber-950/20 px-6 py-2 flex items-center gap-2 text-xs text-amber-700 dark:text-amber-400 shrink-0">
          <span>Using default settings (no rules loaded).</span>
          <span className="text-muted-foreground">Import a settings.json to apply your team&apos;s rules.</span>
        </div>
      )}
          ...
          <div className="space-y-3">
            ...
            {input.team.length === 0 ? (
              <div className="border border-dashed p-6 text-center space-y-1">
                ...
                <p className="text-xs text-muted-foreground">Add manually or import an input JSON file.</p>
              </div>
            ) : (
              <div className="space-y-3">
        ...
        <div className="flex-1 overflow-y-auto p-5">
          {!result && !error && !loading && (
            <div className="flex flex-col items-center justify-center h-full text-center gap-3">
              ...
              <p className="text-muted-foreground/60 text-xs">Import settings.json once for your team rules, then import input.json each month.</p>
            </div>
          )}
          ...
          {result && !isSuccess && (
            <div className="border border-destructive/50 bg-destructive/10 px-4 py-4 space-y-2">
              <p className="text-sm font-medium text-destructive">No valid schedule found</p>
              <p className="text-xs text-destructive/80">Status: {result.status}</p>
            </div>
          )}
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  {result.metadata.objective !== null && (
                    <span className="text-xs text-muted-foreground">
                      Objective: <span className="font-mono text-foreground">{result.metadata.objective}</span>
                    </span>
                  )}
                  <span className="text-xs text-muted-foreground">
                    {result.metadata.solve_time_seconds}s
                  </span>
                </div>
                <Button variant="outline" size="sm" className="gap-1.5"
                  onClick={() => exportScheduleCsv(result.schedule, `schedule-${firstDate?.slice(0, 7) ?? "export"}.csv`)}>
              ...
              <Tabs defaultValue="schedule">
                ...
                <TabsContent value="summary" className="mt-4">
                  <SummaryTable schedule={result.schedule} />
                </TabsContent>
              </Tabs>
            </div>
          )}
