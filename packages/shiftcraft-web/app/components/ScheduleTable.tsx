...
import type { KnownState } from "@/lib/types";

const SHIFT_STYLES: Record<KnownState, string> = {
  ...
  public_holiday: "bg-teal-100 text-teal-800 dark:bg-teal-900/40 dark:text-teal-300",
};

const SHIFT_LABEL: Record<KnownState, string> = {
  morning: "M", afternoon: "A", night: "N", regular: "R",
  week_off: "WO", annual: "AL", comp_off: "CO", public_holiday: "PH",
};

function ShiftBadge({ shift }: { shift: string }) {
  const style = SHIFT_STYLES[shift as KnownState] ?? "bg-muted text-muted-foreground";
  const label = SHIFT_LABEL[shift as KnownState] ?? shift;
  return (
...
export function ScheduleTable({ schedule }: { schedule: Record<string, Record<string, string>> }) {
      ...
      <table className="min-w-full text-sm">
        ...
        <tbody className="divide-y divide-border">
          {dates.map((date) => {
            ...
            return (
              <tr key={date} className={isWeekend ? "bg-muted/20" : ""}>
                ...
                {employees.map((emp) => (
                  <td key={emp} className="px-4 py-2 text-center">
                    <ShiftBadge shift={schedule[date][emp]} />
                  </td>
                ))}
              </tr>
            );
